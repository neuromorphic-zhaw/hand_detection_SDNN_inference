    ...
    def __init__(self, path, joint_idxs=[7,8], cam_id=1, num_time_steps=8):
        ...
        for i, file in enumerate(self.files):
            ...
            for t in range(self.num_time_steps):
                ...
                self.input_frames.append(np.swapaxes(data_dict['input_tensor'][0].to_dense()[:,:,self.cam_id,t].numpy().astype(np.float32), 0, 1)[:,:,np.newaxis,np.newaxis])  # take frame t of cam N, add axis for chanel and time dimension
                # 344 x 260 x 1 x 1
                # 
