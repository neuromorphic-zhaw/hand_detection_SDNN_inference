...
class CustomHwRunConfig(Loihi2HwCfg):
    ...
    def select(self, proc, proc_models):
        ...
        if isinstance(proc, OasisNetEncoder):
            return OasisNetNxEncoderModel
        if isinstance(proc, OasisNetDecoder):
            return OasisNetNxDecoderModel
...
class CustomSimRunConfig(Loihi2SimCfg):
    ...
    def select(self, proc, proc_models):
        ...
        if isinstance(proc, OasisNetEncoder):
            return OasisNetPyEncoderModel
        if isinstance(proc, OasisNetDecoder):
            return OasisNetPyDecoderModel
        return super().select(proc, proc_models)
...
# Input Encoder ###############################################################
class OasisNetEncoder(AbstractProcess):
    """Input encoder process for OasisNet SDNN.
    ...
    """
    # TODO: implement custom process for encoder
    def __init__(self,
                 shape: Tuple[int, ...],
                 net_config: NetDict,
...
@implements(proc=OasisNetEncoder, protocol=LoihiProtocol)
@requires(CPU)
class OasisNetPyEncoderModel(AbstractSubProcessModel):
    """OasisNet encoder model for CPU."""
    # TODO: implement custom subprocess encoder for sim
...
class OasisNetNxEncoderModel(AbstractSubProcessModel):
    """OasisNet encoder model for Loihi 2."""
    # TODO: implement custom subprocess encoder for Loihi
    def __init__(self, proc: AbstractProcess) -> None:
        self.inp: PyInPort = LavaPyType(np.ndarray, np.int32)
        self.out: PyOutPort = LavaPyType(np.ndarray, np.int32)
        shape = proc.proc_params.get('shape')
...
# Output Decoder ##############################################################
class OasisNetDecoder(AbstractProcess):
    """Output decoder process for OasisNet SDNN.
    ...
    shape : Tuple[int, ...]
        Shape of output.
    """
    # TODO: implement custom process for decoder
    def __init__(self,
...
@implements(proc=OasisNetDecoder, protocol=LoihiProtocol)
@requires(CPU)
class OasisNetPyDecoderModel(PyLoihiProcessModel):
    """OasisNet decoder model for CPU."""
    inp: PyInPort = LavaPyType(PyInPort.VEC_DENSE, float)
    out: PyOutPort = LavaPyType(PyOutPort.VEC_DENSE, float)
    # TODO: implement custom subprocess decoder for sim
    def run_spk(self):
...
class OasisNetFixedPtDecoder(AbstractProcess):
    """OasisNet fixed point output decoder."""
    def __init__(self,
                 shape: Tuple[int, ...]) -> None:
        super().__init__(shape=shape)
        self.inp = InPort(shape=shape)
...
@implements(proc=OasisNetFixedPtDecoder, protocol=LoihiProtocol)
@requires(CPU)
class OasisNetFixedPtDecoderModel(PyLoihiProcessModel):
    """OasisNet decoder model for fixed point output."""
    inp: PyInPort = LavaPyType(PyInPort.VEC_DENSE, float)
    out: PyOutPort = LavaPyType(PyOutPort.VEC_DENSE, float)
...
@implements(proc=OasisNetDecoder, protocol=LoihiProtocol)
@requires(Loihi2NeuroCore)
class OasisNetNxDecoderModel(AbstractSubProcessModel):
    """OasisNet decoder model for Loihi."""
    # TODO: implement custom subprocess decoder for Loihi
    def __init__(self, proc: AbstractProcess) -> None:
        self.inp: PyInPort = LavaPyType(np.ndarray, np.int32)
        self.out: PyOutPort = LavaPyType(np.ndarray, np.int32)
        shape = proc.proc_params.get('shape')
        self.decoder = OasisNetFixedPtDecoder(shape=shape)
...
# Monitor #####################################################################
# NOTE: disregard for now - not needed
class OasisNetMonitor(AbstractProcess):
    def __init__(self,
                 shape: Tuple[int, ...],
                 transform: Dict[str, float],
                 output_offset=0) -> None:
        """OasisNet monitor process.
...
@implements(proc=OasisNetMonitor, protocol=LoihiProtocol)
@requires(CPU)
class OasisNetMonitorModel(PyLoihiProcessModel):
    """OasisNet monitor model."""
